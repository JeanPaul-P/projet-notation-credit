...
plt.show()
#Distribution notation de crédit
...
for i, ratio in enumerate(ratios, 1):
    ...
    print(f"Nombre de potentielles valeurs aberrantes pour '{ratio}':", val_aberrantes.shape[0])
    print("Poucentage totale :", round((val_aberrantes.shape[0]/7783)*100, 2), "%")
...
for i, ratio in enumerate(ratios, 1):
    ...
    val_aberrantes = df[(df[ratio] < (Q1 - 1.5 * IQR)) | (df[ratio] > (Q3 + 1.5 * IQR))]
    print("Nombre de potentielles valeurs aberrantes:", val_aberrantes.shape[0])
    print("Poucentage totale :", round((val_aberrantes.shape[0]/7783)*100, 2), "%")

# %%
#Identifier les relations entre variables (corrélations, tendances).
...
# Associer le nombre d'entreprise à la note de Rating score
